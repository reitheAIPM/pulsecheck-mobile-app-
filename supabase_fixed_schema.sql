-- Beta Optimization Database Schema - SUPABASE COMPATIBLE VERSION
-- Uses ::date cast instead of DATE() function for indexes
...
-- SUPABASE COMPATIBLE: Indexes using ::date cast
CREATE INDEX IF NOT EXISTS idx_ai_usage_user_date ON ai_usage_logs(user_id, timestamp::date);
CREATE INDEX IF NOT EXISTS idx_ai_usage_date ON ai_usage_logs(timestamp::date);
...
-- SUPABASE COMPATIBLE: Analytics indexes using ::date cast
CREATE INDEX IF NOT EXISTS idx_feedback_type_date ON ai_feedback(feedback_type, created_at::date);
...
-- =====================================================
-- BETA ANALYTICS VIEWS (SUPABASE COMPATIBLE)
-- =====================================================

-- Daily metrics view for admin dashboard - using ::date cast
...
-- Feedback analysis view - using ::date cast
...
SELECT 
    ...
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (PARTITION BY created_at::date) as percentage_of_daily_feedback,
...
GROUP BY created_at::date, af.feedback_type, af.user_tier
...
BEGIN
    ...
    VALUES (NEW.user_id, NEW.timestamp::date, 1, NEW.total_tokens, NEW.cost_usd, NEW.timestamp)
