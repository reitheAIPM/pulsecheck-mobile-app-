-- Beta Optimization Database Schema - COMPATIBLE VERSION
-- Handles data type mismatch between users.id (UUID) and journal_entries.user_id (VARCHAR)
...
-- Using VARCHAR for user_id to match journal_entries table
CREATE TABLE IF NOT EXISTS journal_summaries (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id VARCHAR(255) NOT NULL, -- Match journal_entries.user_id type
...
-- =====================================================
-- BETA ANALYTICS VIEWS (COMPATIBLE)
-- =====================================================

-- Daily metrics view - compatible with data types
...
-- User engagement metrics view - FIXED to handle data type mismatch
CREATE OR REPLACE VIEW beta_user_engagement AS
SELECT 
    u.email as user_identifier, -- Use email instead of UUID for joining
    u.is_premium,
...
FROM users u
LEFT JOIN journal_entries je ON u.email = je.user_id -- Use email for joining since user_id is VARCHAR
LEFT JOIN ai_usage_logs aul ON u.email = aul.user_id -- Use email for joining since user_id is VARCHAR
WHERE u.created_at >= CURRENT_DATE - INTERVAL '30 days'
GROUP BY u.email, u.is_premium, u.created_at
ORDER BY total_journal_entries DESC;

-- Feedback analysis view - simplified and compatible
...
-- Function to get user tier information - FIXED for VARCHAR user_id
CREATE OR REPLACE FUNCTION get_user_tier_info(p_user_id VARCHAR(255))
RETURNS TABLE (
    user_id VARCHAR(255),
...
BEGIN
    ...
    JOIN user_tier_limits utl ON utl.tier_name = 
        CASE 
            ...
            ELSE 'free'
        END
    WHERE u.email = p_user_id; -- Use email since journal entries use email as user_id
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    RAISE NOTICE '📝 Note: Schema adapted for VARCHAR user_id compatibility';
END $$; 