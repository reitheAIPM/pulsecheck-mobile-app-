-- Beta Optimization Database Schema
-- Add to Supabase database for beta launch preparation
...
CREATE INDEX IF NOT EXISTS idx_ai_usage_user_date ON ai_usage_logs(user_id, DATE(timestamp));
CREATE INDEX IF NOT EXISTS idx_ai_usage_date ON ai_usage_logs(DATE(timestamp));
...
-- Analytics indexes for feedback
CREATE INDEX IF NOT EXISTS idx_feedback_type_date ON ai_feedback(feedback_type, DATE(created_at));
...
-- =====================================================
-- BETA ANALYTICS VIEWS
-- =====================================================

-- Daily metrics view for admin dashboard
...
SELECT 
    DATE(af.created_at) as feedback_date,
    af.feedback_type,
    af.user_tier,
    COUNT(*) as feedback_count,
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (PARTITION BY DATE(af.created_at)) as percentage_of_daily_feedback,
...
FROM ai_feedback af
WHERE af.created_at >= CURRENT_DATE - INTERVAL '30 days'
GROUP BY DATE(af.created_at), af.feedback_type, af.user_tier
...
-- SELECT * FROM beta_daily_metrics ORDER BY metric_date DESC LIMIT 7;

-- Check user engagement
-- SELECT * FROM beta_user_engagement ORDER BY total_journal_entries DESC LIMIT 10;

-- Check feedback sentiment
-- SELECT * FROM beta_feedback_analysis ORDER BY feedback_date DESC LIMIT 10; 