-- Beta Optimization Database Schema - FIXED VERSION
-- Resolves IMMUTABLE function errors in indexes
...
CREATE TABLE IF NOT EXISTS ai_usage_logs (
    ...
    -- Add a date column for efficient indexing
    usage_date DATE GENERATED ALWAYS AS (timestamp::date) STORED
);

-- FIXED: Indexes for analytics queries (using stored date column)
CREATE INDEX IF NOT EXISTS idx_ai_usage_user_date ON ai_usage_logs(user_id, usage_date);
CREATE INDEX IF NOT EXISTS idx_ai_usage_date ON ai_usage_logs(usage_date);
CREATE INDEX IF NOT EXISTS idx_ai_usage_model ON ai_usage_logs(model_used, timestamp);
...
-- FIXED: Analytics indexes for feedback (using stored date column)
CREATE INDEX IF NOT EXISTS idx_feedback_type_date ON ai_feedback(feedback_type, feedback_date);
CREATE INDEX IF NOT EXISTS idx_feedback_user ON ai_feedback(user_id);
CREATE INDEX IF NOT EXISTS idx_feedback_tier ON ai_feedback(user_tier, created_at);
...
-- =====================================================
-- BETA ANALYTICS VIEWS (FIXED)
-- =====================================================

-- Daily metrics view for admin dashboard - FIXED to use stored date column
...
SELECT 
    ...
    COUNT(DISTINCT je.created_at::date) as active_days,
    CASE 
...
WHERE u.created_at >= CURRENT_DATE - INTERVAL '30 days'  -- Beta period focus
GROUP BY u.id, u.email, u.is_premium, u.created_at
ORDER BY total_journal_entries DESC;

-- Feedback analysis view - FIXED to use stored date column
...
SELECT 
    ...
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (PARTITION BY feedback_date) as percentage_of_daily_feedback,
...
FROM ai_feedback af
WHERE af.feedback_date >= CURRENT_DATE - INTERVAL '30 days'
GROUP BY feedback_date, af.feedback_type, af.user_tier
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            WHEN u.created_at >= CURRENT_DATE - INTERVAL '30 days' THEN 'beta'  -- Beta users get special treatment
            ELSE 'free'
        END as tier_name,
        u.is_premium,
        u.daily_ai_usage,
...
BEGIN
    ...
    VALUES (NEW.user_id, NEW.usage_date, 1, NEW.total_tokens, NEW.cost_usd, NEW.timestamp)
...
BEGIN
    ...
    -- This ensures they get enhanced limits during beta period
    UPDATE users 
    SET is_premium = false  -- Keep them as free but they'll get beta tier via the function
